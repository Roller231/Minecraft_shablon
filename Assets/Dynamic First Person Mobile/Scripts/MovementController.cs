...
namespace FirstPersonMobileTools.DynamicFirstPerson
{
    ...
    public class MovementController : MonoBehaviour
    {
        ...
        {
            private get { return m_WalkSpeed; }
            set { m_WalkSpeed = value; }
        } // Accessed through [Walk speed] slider in the settings

        [HideInInspector]
        public float Run_Speed
        {
            private get { return m_RunSpeed; }
            set { m_RunSpeed = value; }
        } // Accessed through [Run speed] slider in the settings

        [HideInInspector]
        public float Crouch_Speed
        {
            private get { return m_CrouchSpeed; }
            set { m_CrouchSpeed = value; }
        } // Accessed through [Crouch speed] slider in the settings

        [HideInInspector]
        public float Jump_Force
        {
            private get { return m_JumpForce; }
            set { m_JumpForce = value; }
        } // Accessed through [Jump Force] slider in the settings
        ...
        } // Accessed through [Acceleration] slider in the settings

        [HideInInspector]
        public float Land_Momentum
        {
            private get { return m_LandMomentum; }
            set { m_LandMomentum = value; }
        } // Accessed through [Landing Momentum] slider in the settings
        ...
        // Input Settings
        [SerializeField] private JoystickMobile m_Joystick; // Available joystick mobile in the scene
        ...
        private Vector3
            m_HeadMovement; // Used for calculating all the head movement before applying to the camera position
        ...
        private Vector2 Input_Movement
        {
            get
            {
                if (m_Joystick != null) return new Vector2(m_Joystick.Horizontal, m_Joystick.Vertical);
        ...
        private void Handle_InputMovement()
        {
            ...
            // #if UNITY_EDITOR
            //         Inputer.x = Input_Movement.x == 0? External_Input_Movement.x : Input_Movement.x;
            //         Inputer.y = Input_Movement.y == 0? External_Input_Movement.y : Input_Movement.y;
            // #elif UNITY_ANDROID
            Inputer.x = Input_Movement.x;
            Inputer.y = Input_Movement.y;
            // #endif

            Vector3 WalkTargetDIrection =
                Inputer.y * transform.forward * m_speed +
                Inputer.x * transform.right * m_speed;

            WalkTargetDIrection = Input_Sprint && WalkTargetDIrection == Vector3.zero
                ? transform.forward * m_speed
                : WalkTargetDIrection;

            m_MovementDirection.x = Mathf.MoveTowards(m_MovementDirection.x, WalkTargetDIrection.x,
                m_Acceleration * Time.deltaTime);
            m_MovementDirection.z = Mathf.MoveTowards(m_MovementDirection.z, WalkTargetDIrection.z,
                m_Acceleration * Time.deltaTime);
        ...
        {
            //Crouching State
            if (Input_Crouch && transform.localScale.y !=
                (m_CrouchHeight / m_CharacterController.height) * m_OriginalScale.y)
            {
            ...
            void CrouchTransition(float TargetHeight, float value)
            {
                ...
                Vector3 Origin = transform.position + (transform.localScale.y / m_OriginalScale.y) *
                    m_CharacterController.height * Vector3.up;
                ...
                transform.localScale = new Vector3(
                    ...
                    Mathf.Lerp(m_OriginalScale.y, (m_CrouchHeight / m_CharacterController.height) * m_OriginalScale.y,
                        m_CrouchTimeElapse / m_CrouchDelay),
